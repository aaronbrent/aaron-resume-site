...
### Phase G — The graphics pass (added 2026-07-19) ✅ = the world stops repeating itself

**Why this phase exists.** The Phase B/C world proved the architecture; a screenshot review (baseline in the session scratchpad) showed where it still reads as programmer art: every tree is the same 6-sided cone, the groomed run is a featureless gradient (worst on the runout), the clouds are two textures reused eleven times, the signs are three boxes, and the town is boxes with prisms. None of that is an architecture problem — baked vertex colors, instancing, and seeded procedural generation take all of these upgrades without new dependencies or assets. **Decision: upgrade in place; do not restart under a new theme.** The learnings (what generalizes to a future hiking/speed-flying/space project) are recorded at the end of the phase.

Order follows perceived impact: trees → mountain → signs/experience areas → clouds → town. Every step stays inside the standing budgets (draw calls ≤ 40, tris ≤ 300 k, chunk ≤ 180 kB gz, zero imported assets, deterministic from `line3d.seed`) and every step is verified against the ride-shots baseline before the next begins.

- [x] **G1 — Trees.** Replace the single cone with 4 merged spruce **archetypes** (3–5 jittered canopy tiers + trunk + snow in one vertex-colored geometry each — one draw per archetype, replacing today's three draws). Bake sun-side/shade-side lighting into the canopy vertices (trees currently ignore the sun entirely); keep instance color for per-tree lightness. Variation comes from archetype choice, non-uniform scale, and a small random lean — not yaw spin, which would rotate the baked light. Scatter upgrade: cluster-based placement (groves + glades instead of uniform confetti), elevation-driven height taper toward a believable treeline, wider height range. _(`rig3d/trees.ts` + `world/scatter.ts`; 2,200 trees, 4 draws.)_
- [x] **G2 — Mountain.** Terrain relief: ridged-noise spurs on the valley walls that grow with distance from the corridor (relief still fades to zero on the groomed run; all Phase B flatness tests stay green). Color bake: concavity ambient occlusion (sampled from `heightAt` at bake time — gullies pool cool shadow), forest-floor darkening under tree clusters so the woods read as woods from far away, and a wind-drift tint band pass on open snowfields. Shader: the contour material grows a **corduroy pass** — fine groomed stripes along the corridor (per-vertex corridor mask + signed across-track meters from the builder), fwidth-faded before moiré, plus a subtle high-frequency snow-grain tint so near snow is never a flat gradient. The corduroy doubles as the missing speed cue. _(The corduroy follows junction decoys along their own centerlines, so forks read as two groomed choices.)_
- [x] **G3 — Signs & experience areas.** Sign furniture: gabled snow-capped roof over the board, diagonal braces, a windswept snow mound at the post feet. Run furniture: **edge markers** (orange-tipped bamboo, instanced, alternating sides every ~26 m of line) — diegetic resort dressing that whips past in the near field, which is where the sensation of speed actually lives. Junction decoys get their own smaller wayfinding boards so the untaken trail reads as a real choice. _(The whole sign kit bakes into one vertex-colored geometry per variant — `rig3d/merge.ts` — so signage is 2 instanced draws, down from 3 pre-G3.)_
- [x] **G4 — Clouds & sky.** Four unique painted cumulus canvases (lobed cauliflower silhouettes with body fill so crowns never bead apart, flat bases, salmon underlight) instead of two reused ones; a flat overhead cirrus veil (horizontal sheets — tilted shell quads read as bars). **Scroll-linked drift**: five layers rotate independently a few degrees across the full descent — the sky moves only while the ride moves, so the parked page still renders zero frames. _(Valley haze band dropped: the dome's below-horizon haze mix already covers it.)_
- [x] **G5 — Town.** Chimneys (snow-capped, merged into one draw), facade palette variety (plaster/cream chalets among the timber), warm **window-glow halos** and streetlamps along the plaza (one shared additive-bloom instanced draw — dusk lights coming on is the golden-hour payoff), and standing chimney smoke columns. _(Smoke stands still — evening air; the drift budget went to the clouds where it reads.)_
- **Exit: passed 2026-07-19** — ride-shots at the baseline t-values verified richer at every stop (tiered sun-lit spruces, corduroy + snow grain in every near field, carved decoy forks, distinct clouds, glowing town); draw calls 31–38 of the ≤ 40 budget (after the merge pass: sign kit 5→2, marker/tower/chimney/cirrus pairs 2→1 each); 67/67 unit + 75/75 e2e green; 3D chunk 144.2 kB gz of 180, critical path 121.1 of 130 untouched.

**Learnings for the next world (recorded for reuse, whatever the theme):** (1) baked vertex color is the whole art budget — sun direction + palette discipline beats materials; (2) one merged vertex-colored geometry per archetype × InstancedMesh is the fidelity/draw-call sweet spot; (3) the near field carries realism — texture the surface the camera hugs before anything on the horizon; (4) reuse is what reads as "programmer art" — N unique painted canvases cost the same init code as 2; (5) scroll-linked motion gives "alive" without an idle rAF cost. These transfer directly to a hiking/ski-touring/speed-flying/space variant, which stays a **future-work option** (§12), not a replacement for this world.
...
- **A second world, different theme (noted 2026-07-19).** The Phase G learnings (baked-light vertex color, archetype instancing, near-field-first texture, scroll-linked ambient motion) are theme-agnostic — a hiking traverse, ski tour, speed-flying line, or orbital descent could reuse the entire rig + LUT + tier system with only the world modules swapped. Kept as an option for a future project, not a replacement for this one.
