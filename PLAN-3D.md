...
**How to use this doc:** same contract as PLAN.md — every phase ends demoable, exit criteria are gates, sections 1–8 are decisions, 9–11 are work/risks/open questions.
...
| Loading              | 3D is a **lazy, code-split chunk**; the prerendered document + 2D map paint first and never wait on it                                                                                         |
...
├── <SummitHero>                     server · unchanged — LCP element, gondola intro intact
...
| Spray                   | GPU points from the lee edge, rate = speed ×                                        | curvature                   | (v1's emitter law, verbatim) | 2,048 pool, one draw call |
...
**Audio (optional, Phase E):** wind + edge-carve loop, gain from speed. Off by default, patrol-orange unmute toggle in the legend, `muted` persisted. Never autoplays with sound — this is a resume opened in offices.

---

## 6. Loading & the transition

1. **Paint:** prerendered document + hero + 2D map — identical to v1, LCP unchanged, zero wait on 3D.
2. **Warm:** after `requestIdleCallback` (and only if the tier gate says 2), fetch the 3D chunk, create the context, build terrain + LUT, prerender one frame off-screen.
3. **The drop-in:** when ready, the 2D map **tilts and dives** — CSS perspective rotation on the existing SVG stage crossfading into the 3D camera descending from a matched aerial angle to the summit lip POV. The metaphor is literal: _the printed map stands up and you land on it._ If the visitor has already scrolled, skip the ceremony: fade at their current t (scroll is never gated — v1's gondola rule).
4. **Failure at any step:** stay in Tier 1 silently. No spinner ever appears over the map; the 2D site _is_ the loading state.

The gondola credits stay in the hero (CSS, tier-agnostic). Stretch (Phase E, cut-eligible): the intro traverse passes under the lift line and the credit signs hang from the towers.
...
| Lighthouse script budget (`budgets.json`)                            | 130         | 320 (idle-loaded chunk counts; documented)                                                      |
...
- [ ] Terrain benches at dwell zones stamped by the generator; init-time budget measured (< 20 ms or move to build-time — decision gate)
...
### Phase E — The drop-in & polish (days 18–20)

- [ ] Map-tilt dive-in transition (skippable by scroll, never gates)
- [ ] Live tier demotion (context loss / sustained frame misses → Tier 1 at same t) + e2e for context-loss path
- [ ] Audio (optional, muted default) — cut-eligible without ceremony
...
- **Exit:** cold load → hero → dive-in → full run → hockey stop → contact, on a phone, no instructions, no jank.
...
- [ ] OG image refresh (a POV frame with a sign — or keep the map card; decide by which reads better at 1200×630), meta copy pass
...
| 9   | **In-app WebViews mis-tier** (LinkedIn/iMessage — the actual first-visit surfaces) | Tier gate probe results from the Phase F matrix           | Conservative gate defaults; probe is 3 s and falls to Tier 1 on any ambiguity — the fallback is the finished v1, so a false negative costs almost nothing   |
...
2. **Sound.** Wind/carve audio lands well in POV but this is a resume opened at desks. Default-muted toggle planned; is audio worth Phase E days at all, or cut now?
3. **OG image:** keep the trail-map card (proven, on-brand) or a POV frame with the SoFi sign (shows the new thing)? Needs the 1200×630 squint test.
4. **The drop-in ceremony length.** Map-tilt dive target ≤ 2.5 s and skippable — confirm the appetite, since it runs on every cold visit.
5. **Gondola credits on lift towers** (stretch): worth it, or does the hero gondola stay the sole home of the education story?
6. **WebGPU** as a post-launch renderer swap behind the same rig — park it in the same bucket as v1's night-skiing mode?
7. **Domain/analytics** — v1 open questions #2/#3 remain open and unchanged by this plan.
